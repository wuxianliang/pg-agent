...
def expect(t,fn):
 try: fn()
 except QueryValidationError as e: check(t,e.envelope['Type'] in {'DUCK_READ_ONLY_VIOLATION','DUCK_EXTERNAL_ACCESS_FORBIDDEN','DUCK_PARSE_ERROR','DUCK_ARGUMENT_ERROR'},e.envelope); return
 raise AssertionError('expected '+t)
def main():
 check('setup',setup_db()==0)
 c,_=bootstrap_connection(GrammarExtensionConfig()); c.execute('create table t(x int, y int)'); c.execute('insert into t values (1,10),(2,20)')
 ...
 with pg.cursor() as cur:
  ...
  cur.execute("SELECT value FROM prompt_parts WHERE recipe_name='agent_system' AND recipe_version=3 AND slot_key='task'"); row=cur.fetchone(); task=str(row[0]) if row else ''; check('DuckDB guidance in recipe','wb_duck_register' in task,task); check('static recipe is pipe-neutral','|>' not in task and 'pipe_query_syntax' not in task,task)
 expect('missing connection',lambda: validate_read_query('SELECT 1', None))
