...
def validate_read_query(sql: str, con: duckdb.DuckDBPyConnection | None = None) -> ValidatedQuery:
    ...
    if con is None:
        raise QueryValidationError('DUCK_ARGUMENT_ERROR','a bootstrapped managed DuckDB connection is required','Pass the current v6 session connection; do not validate on an implicit connection.')
