-- ============================================================
-- PG-Agent v4 · plugin_sql_curator（策展 TEMP VIEW：定义 + sticky run 连接备注）
--
...
{"plugin":{"name":"plugin_sql_curator"},"llm_tool":{"name":"wb_sql_curate","description":"在当前 sticky run 连接定义/替换一个带备注的策展 TEMP VIEW：委托 wb_temp_view_create 校验并创建（p_select_sql 单条 SELECT/WITH，上限 8000 字符，比生命周期工具更严）；p_note ≤1000 字符存为视图备注（wb_temp_view_list 的 note 可见）；省略/NULL/纯空白 p_note 表示替换视图并清除旧备注","args":{"p_view":"text","p_select_sql":"text","p_note":"text"},"returns":"jsonb","session_scope":"run_connection","capability":"temp_view_mutation"}}
$wb$;
