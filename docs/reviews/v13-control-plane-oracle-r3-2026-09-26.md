...
## §8 R3c 补裁（2026-09-26 第四轮：P2 机器形状 H1–H7 + P3/P4 前向扫掠）

- 触发：P2 实施轮（Turn 5）双车道 STOP 于 6 洞+1 分歧。三车道（grokBuild/openCode/codex）均裁 **P2 GO**；形状差异由控制器合并，少数意见存 §8.7。全文：`docs/reviews/v13-control-plane-oracle-r3c-full-2026-09-26.md`。R2 A17/R3*/已裁不重开；H1/H3 为对 R2 §2.3/扇出字面的**显式收窄**。

### 8.1 H1 预算算术（收窄 R2 §2.3：席位计数，非 turn 算术）

活体无 max_turns/reserved 列；turn_budget.max_cycles 是本 user turn 路由环上限、turn_no 是 user/message 计数（R1.11 禁 spawn 改），把任一接成树预算=第二本账，**禁止**。替代：策略行 `v13_policies` 名 `spawn_budget` v1 active，值恰三键 `{max_nonterminal:8, max_depth:4, max_fanout:8}`（JSON 整数≥1；未知/缺/越界 RAISE `v13: spawn_budget policy`）。`requested`=children 数组长度；非终态=status NOT IN 三终态（含 blocked_unknown）；预算根=沿 parent 走到 NULL（环或 >64 跳 RAISE，不取锁）；占用=根全部子孙中非终态数（不含根）；准入=`占用+requested≤max_nonterminal 且 parent_depth+1≤max_depth 且 1≤requested≤max_fanout`，失败 RAISE（尾词 cap/depth/fanout）零 INSERT。锁：仅 v13_spawn_subsession 在父行锁后 `pg_advisory_xact_lock(v13_advisory_class('spawn_budget'), hashtext(root::text))`（两参 int 形式，持到事务尾）；`v13_advisory_class(name)→int` IMMUTABLE，闭集 `'spawn_budget'→13001`，控制面类号只许 13001–13099 只许改此函数（禁哈希类号）；count 在锁后、插完全子后复查仍超则 RAISE；fork/closeout/recover/cancel 不取此锁；禁 sessions.reserved。

### 8.2 H2 spawn 入参与回执（合并案）

`v13_spawn_subsession(p_sid uuid, p_spec jsonb) → jsonb`，SECURITY DEFINER 属主同三入口，父锁重入或自锁；模型不可提供 parent/cutoff/kind/session_id/count/max_turns。**入参闭集**：`{schema_version:1, children:[{tool_call_id, task}]}`——tool_call_id 文本 1..128 `^[A-Za-z0-9_./:-]+$` 批内唯一；task 文本 1..1024；数组长度见 H1；未知键/空数组/缺键 RAISE `v13: spawn args` 零行。派生：kind='fresh_fork'（不复制 latch；worktree 不遗传）；cutoff=next_seq-1（next_seq≥1 否则 RAISE requires prefix）；父 status 必须 ready|waiting（终态/墙 RAISE）；fork 后子若非 ready 则 UPDATE ready（waiting 不遗传）。分工：fork 仍是唯一 INSERT sessions 原语（换属主不改逻辑）；spawn 做准入/幂等/循环调 fork/写父事件；不入队不 send_work 不插 effect 不写 tool/result。**回执事件**：父 `child-created`（R2 字面，不用斜杠；source_effect_id=对应 effect 非空）恰键 `{schema_version:1, tool_call_id, spawn_kind:'fresh_fork', cutoff, tasks_hash, children:[{session_id,task}]（同序）, reservation:{requested,nonterminal_before,max_nonterminal,parent_depth}}`；tasks_hash=sha256(按序 task 字符串数组的 jsonb 文本)。每子 `spawn/task`（source_effect_id 必空）恰键 `{schema_version:1, tool_call_id, task}`——不写 user/message、不改子 turn_no、不 advance 子。**返回恰键**：`{replay_kind:'created'|'replay', parent_session_id, tool_call_id, spawn_kind, cutoff, children:[uuid], tasks_hash, reservation(同上四键)}`。幂等：父上同 tool_call_id 的 child-created 存在且 tasks_hash 相同→零写返 replay（reservation 抄事件原文）；不同→RAISE mismatch；部分唯一索引 `(session_id,(payload->>'tool_call_id')) WHERE type='child-created'`。失败只有 RAISE。

### 8.3 H3 批量扇出（收窄：权威=tool/call 事件，生产者=llm 结果；openCode 案）

R3a 十键 additionalProperties:false 禁为扇出加键（harness 结果出现 tool_calls=未知键拒）。**生产者**：`v13_complete` 写行前，kind=llm 且 succeeded 且 result 含 `tool_calls` → 校验后同事务按序各追加一条 `tool/call`（source_effect_id=该 effect）；缺键=今天 llm 路径零变化；形状坏 RAISE 零写不烧 attempt/fence。`tool/call` payload 恰 `{schema_version:1, id, name:'spawn_subsession', args:{}}`（id 规则同 tool_call_id；name 不符或 args 非空→整批 RAISE）；唯一索引 `(session_id,(payload->>'id')) WHERE type='tool/call'`。**advance 切入**（会话锁内、步 0/墙/cancel 之后、P0 finish 之前）：本 user turn（seq>last_user_seq）未消费 tool/call（无同 id 的 child-created）合成**一批**；零条→旧管线；有则不走 finish、不走通用 sql EXECUTE/EXCEPTION 臂，直接调 v13_spawn_subsession；成功后写一条 turn/route `{action:'sql',reason:'spawn_fanout',tool:'spawn_subsession',params:{}}` + 一条 succeeded tool effect（request=v13_spawn_request 合成，身份走 v13_effect_id）+ 一条 tool/result（既有形状，result=函数返回值）；任一步 RAISE=整笔回滚零子；返 progressed。N 或 0，无部分子。route 排除：v13_needed_judgments 活体排除 spawn 名（v13_is_spawn_tool 单源字面量，同 v13_is_harness_tool 手法）；sql 臂命中该名→RAISE batch-dispatched 不落 failed effect。

### 8.4 H4 v13_recover_idle（grokBuild 案）

驱动提示非验收：禁调 parse/advance/closeout/send_work、禁写 session/*·turn/end·material、禁改 status、禁插 effect、禁取 spawn 锁；子 closeout 事务零父事件。空闲=status∈{ready,waiting} 且无 ready|claimed|unknown effect 且非墙。三类原因（可同时，各一条）：`children_terminal`（直接子≥1 且全终态；fingerprint=sha256(按 session_id 升序 [[id,status]…] jsonb 文本)）；`repair`/`replan`（存在对应 required 事件；fingerprint=该 type 全部 seq 升序数组文本；不做消费减法）。事件 `recover/nudge`（source NULL）恰键 `{schema_version:1, reason(上三值), fingerprint}`；唯一索引 `(session_id,reason,fingerprint)`；重插拒绝。锁：候选 session_id 升序 **FOR UPDATE SKIP LOCKED**（扫地僧与 advance 不死锁，丢一拍靠下次）；锁内复验。返回恰键 `{pending(uuid 升序), nudged(本调用新插数)}`；第二次同指纹 nudged=0 零 effect。调用者=cron/驱动，提交后对 pending 各做一次 parse+advance；children_terminal 求值在 stage 18 激活（去 P1 RAISE，按 R3a §6.2 直接子全终态）。

### 8.5 H5 v_goal_tree + H6 收据 children + H7 存放位

**H5**（三车道一致）：`v_goal_tree(p_root uuid)` plpgsql STABLE SRF 非 VIEW；列序恰 `session_id uuid, parent_session_id uuid, depth int, status text, spawn_kind text, turn_no int, is_terminal boolean`；根 depth=0 含根；未知根 RAISE；环 RAISE；depth>64 RAISE；ORDER BY depth,session_id；EXECUTE 授 recall/resolve/route；无预算列无终态时间。
**H6**（合并案，三车道均否决保留 RAISE）：stage 18 换体 v13_closeout（以 pg_get_functiondef 活体为底；活体无 `children require stage 18` 子串则停）：删该 RAISE；公共前置改「无非终态**直接**子」（有则 RAISE precondition 尾词 children_open，**含 cancel/预算逃生也拦**——P2 不扇出不能把活子树封进父终态）；advance 各终态臂遇非终态直接子：不调 closeout、不向驱动 RAISE（防重试风暴）、非墙则置 waiting 返 waiting，子齐后 nudge+下次 advance 封账。零直接子→children:[]；全终态→按 session_id 升序填 `[{session_id,status}]`（status 三终态；无孙无 state_hash 无 spawn_kind）；**schema_version 保持 1 不升版**（键集未变；v1⇔[]/非空⇔有子由 payload 自述）；state_hash 算法一字不改。**H7**（三车道一致：函数体字面量，禁列/param_spec/策略行）：`v13_named_sql_writer(handler)→jsonb` IMMUTABLE 单源，P2 闭集仅 `v13_spawn_subsession`（裸名/public. 前缀），返回恰 `{"mutating":false,"write_targets":["artifacts","events","latches","sessions"]}`（字典序），其余 NULL（NULL+VOLATILE=拒）。stage 18 换体 v13_tools_guard（活体为底）：kind=sql AND enabled AND provolatile='v' 且名单外→RAISE；名单内须同时：签名 (uuid,jsonb)→jsonb、prosecdef、proowner=属主、search_path 恰 pg_catalog,public、public 无 EXECUTE 且 v13_route 有、lower(prosrc) 禁 dblink/pg_net/copy program/lo_import/lo_export/pg_read_file/pg_write_file、write_targets 等于四名（非子集）。tools 行：name='spawn_subsession', kind='sql', handler='v13_spawn_subsession', param_spec='{}', enabled=true，description 纯 ASCII 无 `::`；先 REVOKE 再 INSERT（guard 查 ACL）；INSERT 会拨 tools_revision（gate 相对比较）。v13_fork 签名 (uuid,bigint,text,jsonb) 不作 sql 工具不插行。

### 8.6 P2 交付清单（叠加 R3 §3）与顺序

`v13/spawn/v13_spawn.sql` 单文件自包事务，内部序：策略行 spawn_budget→谓词/索引/新事件守卫 `v13_spawn_event_guard`（WHEN 限 spawn/task·child-created·tool/call·recover/nudge；不 REPLACE P1 守卫）→属主 NOLOGIN NOSUPERUSER+两角色+INSERT 闭集触发器（current_user=属主）+fork 换属主（活体为底只换 owner，触发器在三函数就位后才 CREATE）→v13_open_session（R3 §1 附四收紧）→v13_spawn_subsession→complete 换体（llm tool_calls→tool/call 投影；children_terminal RAISE 移除）→wake 求值器激活 children_terminal（R3a §6.2：直接子存在+parent=本会话+全终态，缺失/非直接子/重复 RAISE）→advance 换体（扇出臂+终态臂停泊）→closeout 换体（H6）→v_goal_tree+recover_idle→guard 换体→tools 行。既有直插测试改走 v13_open_session。安装自检：SET ROLE 属主调 open 成功、v13_route 直插失败。gate：G-spawn-unique-writer（三函数+误授 INSERT 仍拒）、G-open-session-shape、G-sql-write-closed、G-spawn-fanout（N 或 0；tool_calls 坏形状整批拒）、G-ctx1-spawn、双根互不占、墙上子仍占席位、children_terminal wake 正负例、recover/nudge 幂等与 SKIP LOCKED、收据 children 两态+children_open 拦 cancel、v_goal_tree 列序/环/深度。

### 8.7 少数意见与 P3/P4 前向默认

- 少数意见：codex H1 键名 max_active_descendants/max_batch_children（种子 16/32）与类号 13018、H2 无 task 正文、H3 事件名 child/created 与 orchestrator-result 生产者——被合并案否；grokBuild H3 spawn/batch+外部提交器与 sql 臂 EXECUTE——被 openCode llm-result 案否（更贴 R2 tool_call 语义）；openCode H6 升 schema_version=2——被不升版案否。
- **P3/P4 前向默认（开工不得再当洞；与 ch08/R2 字面冲突则停）**：cancel 扇出=每会话一条既有 cancel/requested（scope 仍 session），应用序 depth 升/同层 id 升，锁序全部 id 升序锁完再写；`v13_cancel_pending(sid)→boolean` STABLE（谓词抄 R3b），worker 每次 renew 成功后轮询（无 LISTEN 无新队列）；`v13_interruptible(name)→text` 字面量 required|best_effort|unsupported 缺省 unsupported，stage 19 才给 complete 加 p_status=cancelled（条件：未消费 cancel 且 best_effort 或 required+llm；kind=tool+required 禁 cancelled 须 unknown 走 P1 抬墙）；FS effect=**kind=tool** 三目录名 worktree_prepare|merge|release（handler worker:*，params 恰 binding_artifact_id；禁新 kind 禁 sql）；worktree latch 名 'worktree'，value `{schema_version:1, binding_artifact_id, state:prepared|released}`，artifact kind='worktree_binding'（活体有 CHECK 则 stage 19 ALTER 追加值；不容则 latch-only 降级并另裁）；`v13_requires_worktree(name)` 仅 merge/release true（prepare 自建 binding；勿全 tool 都要 binding）；`v13_latch_identity_excluded` P3=['worktree'] 换体 latch_digest 排除；fork 非 fresh 复制跳过 worktree。P4：goal/override 恰 `{schema_version:1, intent:direct|decompose, source_principal:user|operator, reason 0..256}`，唯一写者 v13_submit_override；thresholds.action 保持 pass|reject（六值指路由出口禁 ALTER）；`v13_triage_decide(proj)→direct|decompose|human|reject|none` 纯函数+v13_triage_project 组装（null 不点火；has_mutating_hint 恒 false）；duty_cycle 策略行 triage {duty_cycle:1}（0→写 triage/hold 置 waiting）；direct 时 route 选 spawn→改 human reason triage_direct；explore=同会话 llm route.reason='explore'+事件 explore/completed {schema_version:1,evidence_hash}（本 user turn 一次；不占 depth/席位）；fold cap=thresholds 行 signal 恰 harness.repair_count/harness.replan_count（计数=事件条数；超限→human request 恰 {reason:'repair_cap'|'replan_cap'} 同 DP1 {reason} 形不走 C4；已答按 R3a cap-human 新逻辑转）；material_cap P4 仍不生产（超限 material 续 RAISE）。仍不做：closeout/inbox_residual、quota/spent·voided、G6 新条文。**另裁触发**：拒绝 latch-only 降级、拒绝轮询第四务、要把「凡外部 mutating harness」都纳入 requires_worktree——仅此三项需 P3/P4 开工前叫 Oracle。

### 8.8 接缝修正（2026-09-26 控制器；修复 §8.2×§8.3 合并伤，非新裁决）

实施轮 STOP 指出 §8.2（grokBuild 单批回执）与 §8.3（openCode 逐子 tool/call）接缝不齐：单数 tool_call_id 在 N>1 无来源、一条回执消费不了 N 个 tool/call、task 无事件源。修正（**§8.2/§8.3 与本节冲突处以下列为准**，均回到 openCode 原案不发明）：

1. **llm `tool_calls` 元素形状**：恰 `{id, name:'spawn_subsession', args:{task}}`——task 文本 1..1024（args 不再是空对象）；name 不符或 args 缺 task/多键→整批 RAISE。`tool/call` 事件 payload 镜像恰 `{schema_version:1, id, name:'spawn_subsession', args:{task}}`；唯一索引不变。
2. **逐子回执**：`child-created` **每子一条**，payload 恰 `{schema_version:1, tool_call_id, child_session_id, spawn_kind:'fresh_fork', cutoff, reservation:{nonterminal_before, requested, max_nonterminal, parent_depth}}`（reservation 四键为批级准入快照，同批各条相同；无 tasks_hash、无 children 数组、无批级单数 id）。`source_effect_id`=advance 预计算的合成 spawn tool effect id。唯一索引仍每子一条，故 N 个 tool/call 被 N 条回执逐个消费。
3. **task 源**：advance 批臂从对应 tool/call 的 `args.task` 逐字抄进入参 children；spawn/task 每子事件 `{schema_version:1, tool_call_id, task}` 不变。
4. **返回闭集改为**：`{replay_kind:'created'|'replay', parent_session_id, children:[{tool_call_id, session_id, task}], reservation}`（无 cutoff/spawn_kind/tasks_hash 键——cutoff 在各回执里）。
5. **幂等改判**：重调时每个请求 tool_call_id 都已有 child-created 且子行在且**存储 task 与请求 task 逐字相等**→零写返 replay；部分已消费→RAISE `v13: spawn ledger`；task 不等→RAISE mismatch。
6. **未消费/批定义**：未消费 tool/call=无同 id 的 child-created；批=本 user turn 全部未消费 tool/call（seq 序）；零条→旧管线；部分已消费则剩余者仍为批（各自独立消费）。
7. **v13_spawn_request 键序冻结**：恰 `{tool:'spawn_subsession', params:{children:[{tool_call_id,task}…]}}`，advance 用它算 effect_id；child-created 在 handler 内写、effect 行插在 handler 返回后，两处 id 相同。
