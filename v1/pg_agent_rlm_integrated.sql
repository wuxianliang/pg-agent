-- ============================================================
-- PG-Agent · RLM × CodeAct 整合层
--
-- 依赖：pg_agent_functional.sql（agent_runs / agent_steps / jobs /
--       handlers / http_call_llm / exec_sql_readonly / emit_step /
--       parse_llm_output / fold_messages / worker）
-- 建议：同时加载 pg_agent_poml.sql（提供 agent_loop，codeact_spawn 会用到）
--
-- 共用数据基础：
--   agent_runs   —— 加 paradigm/parent/depth，CodeAct 与 RLM 同一张 run 表
--   agent_steps  —— 同一种追加日志，run_state() 对两种范式都成立
--   jobs/handlers/worker —— 同一队列，job_type='rlm_run' 即可派发
--   http_call_llm / exec_sql_readonly —— 同一套 LLM 与只读 SQL
--
-- 两个入口：
--   agent_run_rlm     —— 纯 RLM（上下文进 rlm_vars，REPL 里 spawn）
--   agent_run_hybrid  —— CodeAct 主循环，可 action=rlm 把子任务交给 RLM
-- RLM 侧也可 SELECT codeact_spawn('...') 把简单查询交给 CodeAct。
-- ============================================================
