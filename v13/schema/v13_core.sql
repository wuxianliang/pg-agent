...
--
-- Source: docs/plans/v13-dp1-two-phase-advance-plan-2026-09-20.md §3.1
-- (turns 3-12 fixes already folded into that draft), plus the later
-- amendments: 「v2 对齐修订」A1/A3/A5 (registration only, zero DDL delta)
-- and 「R2 对齐修订」#50/A17 (v13_tools_guard named VOLATILE exception).
-- Mechanical deviations from the plan text are listed in README.md.
...
-- 未来增列缝(v2 对齐修订「sessions 未来增列缝」/R0a):ws_id、files_cutoff
-- 走 SQL_LOAD_ORDER 纯末尾追加,不在本文件;ws_revision 明文不加(A3 禁第八键)。
...
-- 绑定)——它引用的 v13_policy 定义在更后方(策略段),sql 语言体会在
...
-- (3) **R2 #50/A17 具名 VOLATILE 例外闭集(2026-09-21 控制面终裁)**:默认仍
--     拒 VOLATILE;具名核心函数闭集 (v13_fork / v13_spawn_subsession /
--     rag_assemble) 在**五条件全满足**时放行——① provolatile='v';② 精确签名
--     (uuid,jsonb)→jsonb;③ SECURITY DEFINER + 固定 search_path + owner=控制
--     角色(DP1 的控制角色=v13_route);④ GRANT EXECUTE 只给控制角色,模型/
--     只读角色族无 EXECUTE;⑤ prosrc 不含 dblink/pg_net/COPY … PROGRAM 等
--     IO 通道。闭集外的 VOLATILE 一律仍拒(M1-14 断言语义不弱化);扩员=
--     guard 源码改 + 设计修订 + 部署 gate 同发,禁 UPDATE tools 扩员。
--     M1 阶段闭集三函数均不存在 ⇒ 例外分支不可达,默认拒面完整生效。
...
BEGIN
  ...
  IF NEW.kind = 'sql' AND NEW.enabled THEN   -- 只校验 enabled 行(turn 8,#53)
    ...
    IF NOT EXISTS (
      ...
      -- R2 #50/A17 具名闭集例外:五条件全满足才放行(闭集外/条件不全→仍拒)
      IF NOT EXISTS (
        SELECT 1 FROM pg_proc p
         WHERE p.proname = NEW.handler
           AND NEW.handler IN ('v13_fork','v13_spawn_subsession','rag_assemble')
           AND oidvectortypes(p.proargtypes) = 'uuid, jsonb'
           AND p.prorettype = 'jsonb'::regtype
           AND p.provolatile = 'v'
           AND p.prosecdef
           AND array_to_string(p.proconfig, ',') LIKE '%search_path=%'
           AND pg_get_userbyid(p.proowner) = 'v13_route'
           AND has_function_privilege('v13_route', p.oid, 'EXECUTE')
           AND NOT has_function_privilege('public', p.oid, 'EXECUTE')
           AND NOT has_function_privilege('v13_recall', p.oid, 'EXECUTE')
           AND p.prosrc !~* '(dblink|pg_net|copy[[:space:]].*program)') THEN
        RAISE EXCEPTION
...
--     这类不进判断哈希的列)→ stale 重解析。turn 9,#59 增列
--     candidate_generation_revision:needed 集=f(tools 行集,
--     **v13_needed_judgments 函数体**)——OR REPLACE 换推导体不触 tools 行、
--     revision 不动(六键论证的破绽);由文末 DDL event trigger 第二分支命中
--     目标名 bump(两键独立:目录行 DML 只动 revision,needed 体 DDL 只动
--     cgr;DP5 语料版本面必须并入本键或另立,§1.3 硬契约)。
--     **A3/D2 登记(v2 对齐修订)**:探针七键零结构增量;未来 workspaces
--     作用域列 DML 并入 tools_revision bump + 'ws_scope_changed' 审计事件,
--     **禁第八键 ws_revision**。 ===
...
-- jsonb_build_object/::jsonb,无第二例字符串拼接)
 ...
 ('effect_attempt_cap',1,
  '{"judge":4,"tool":3,"llm":3,"human":2,"context_refresh":3}'::jsonb, true);
                                                   -- 末元组分号=turn 9 机械修:
                                                   -- 缺则与下文 CREATE TABLE
                                                   -- 粘连(42601)。按 kind 的
                                                   -- attempt 上限(turn 7,#49
                                                   -- 重挂+turn 8,#55 扩至
                                                   -- requeue 回收/claim 领取
                                                   -- 共用同一判定;judge 4>
                                                   -- resolve_retry 2——常态由
...
--     'probe_unreachable')。运维可人工补注(origin='manual')——补注同样
--     不改变 α 行为。**SQL 面零消费:不授任何角色 SELECT,owner/监控专用**
--     (吸收面与注册解耦,注册永不放大吸收)。 ===
...
-- 机械补授(草案矩阵缺口):route 还需 **SELECT effects**。enqueue/claim/
-- complete 全在 route 手,而 `SELECT * INTO v_row FROM effects WHERE …` 与
-- `UPDATE effects … WHERE effect_id=…` 的 WHERE/RETURNING 均需 effects 的
-- SELECT 权限(PG:UPDATE 的 WHERE 引用列要 SELECT)。草案的
-- 「route = recall ∪ {INSERT/UPDATE effects…}」把 effects 留在 recall 的
-- 只读七表之外,导致 M1-7「route → EXECUTE v13_enqueue_effect ✓」不可达。
-- 只给 route(recall/resolve 仍无 effects 读权,M1-7 的 resolve 负向不变)。
GRANT SELECT ON effects TO v13_route;
...
-- A1(v2 对齐修订 + R2 收窄):快路 handler = 库内、零外部 IO、零队列;
-- 零 FS/git IO;默认只读,库内写仅 R2 #50 具名闭集。
...
--     'uuid, jsonb'(目录守卫钉死的唯一合法契约签名);handler 文本认裸名与
--     schema-qualified 名两种形态(与目录写入形态一致)。
...
--     表/类型才填充),不可用**);本面不做签名精确过滤:同名异参 overload
--     的 DROP 保守 bump 一次(单调计数器,后果=保守弃批,无害)。
...
-- 求值(只捕 39P03 上下文违约,不吞其他错误)。
-- ROUTINE 同义族 tag(turn 11 收敛;turn 12 名单同步收口):ALTER ROUTINE/
-- DROP ROUTINE 是独立 command tag 且可作用于函数(实测 PG18.4)。**tag 名单
-- 两处必须逐 tag 同步**:trigger 的 WHEN 名单(放行)与函数体内面 (a) 的
-- command_tag 过滤名单(消费)。'CREATE ROUTINE' 一员系 belt 预置:实测
-- PG18.4 无 CREATE ROUTINE 拼写(语法拒),该 tag 当前不可达;未来引擎放行
-- 此拼写则名单已含、无需改 SQL(M1-13 语法拒负向断言锚住)。
-- PROCEDURE 族不涉:prorettype=void 的过程过不了目录守卫。残余面:DROP
-- SCHEMA … CASCADE/DROP OWNED 的顶层 tag 不命中列表、不 bump revision——但
-- handler 已随级联消失,下一次 parse 的冻结校验路径 RAISE(fail-closed)。
-- 双通道互斥语义(turn 9,#59):分支 1 命中 handler 集(tools 行 kind='sql'
-- 的 handler 文本);分支 2 只配 v13_needed_judgments(M2 对象;M1 阶段不存在
-- ⇒ cgr 恒不动)。needed 分支按名称匹配不限 schema(漏报是危险方向)。
-- 置于文件末尾:本文件自身的 CREATE FUNCTION 先于触发器存在,不触发。
-- SECURITY DEFINER+钉 search_path=event trigger 在任意用户 DDL 期间触发,
-- 须以属主权限读 tools/meta 且免搜索路径劫持;REVOKE PUBLIC(直调在事件
-- 上下文外本就报错,双保险)。**部署前置:CREATE EVENT TRIGGER 需超级
-- 用户**(与上方 DO 块 CREATE ROLE 的权限面同属 setup_db 前提)。
